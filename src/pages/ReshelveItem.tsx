...
const steps = ['Enter Item ID', 'Check Damage', 'Process Return', 'Confirmation'];
...
export const ReshelveItem: React.FC = () => {
  ...
  const [transaction_info, set_transaction_info] = useState<TransactionInfo | null>(null);
  ...
  React.useEffect(() => {
    if (selectedBranch) {
      set_return_branch_id(selectedBranch.id);
    }
  }, [selectedBranch]);
  ...
  const scan_and_identify = async () => {
    ...
    try {
      ...
      const tx_response = await fetch(`${API_BASE_URL}/transactions?status=active`);
      ...
      const transaction = all_transactions.find((tx: any) => tx.copy_id === parseInt(item_id_input));
      ...
      const patron_response = await fetch(`${API_BASE_URL}/patrons/${transaction.patron_id}`);
      ...
      if (patron_response.ok) {
        ...
        patron_name = `${patron.first_name || ''} ${patron.last_name || ''}`.trim();
      }

      // Get item details from LIBRARY_ITEM_COPIES table
      const copy_response = await fetch(`${API_BASE_URL}/item-copies/${item_id_input}`);
      ...
      const days_overdue = is_overdue 
        ? Math.ceil((current_date.getTime() - due_date.getTime()) / (1000 * 60 * 60 * 24))
        : 0;
      
      // Calculate late fee ($0.50 per day)
      const late_fee = days_overdue * 0.50;
      ...
      set_error(err instanceof Error ? err.message : 'Item not recognized - not in system');
  ...
  const complete_return = async () => {
    ...
    try {
      ...
      if (is_damaged) {
        const damage_response = await fetch(`${API_BASE_URL}/item-copies/${transaction_info.copy_id}`, {
          method: 'PUT',
          headers: {
        ...
        if (!damage_response.ok) {
          ...
          throw new Error(error_data.message || error_data.error || 'Failed to mark item as damaged');
        }

        console.log('Item marked as damaged, now checking in...');
        
        // Then check in the transaction (no late fees for damaged items)
        const checkin_response = await fetch(`${API_BASE_URL}/transactions/checkin`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (!checkin_response.ok) {
          ...
          throw new Error(error_data.message || error_data.error || 'Check-in failed');
        }
      ...
      set_error(err instanceof Error ? err.message : 'Failed to complete return');
    } finally {
      ...
      <Paper elevation={3} sx={{ p: 4, flex: 1 }}>
        ...
        {active_step === 0 && (
          <Box>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
              Enter the item's copy ID and select the branch where it's being returned.
            </Typography>
        ...
        {active_step === 1 && transaction_info && (
          <Box>
            ...
            <Box sx={{ mb: 3, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
              <Typography variant="body1" fontWeight="bold">Item: {transaction_info.item_title}</Typography>
              <Typography variant="body2">Copy ID: {transaction_info.copy_id}</Typography>
            </Box>

            <Box sx={{ mb: 3, p: 3, border: '2px solid', borderColor: 'warning.main', borderRadius: 2 }}>
          ...
          <Box>
            ...
            {is_damaged && (
              <Alert severity="error" sx={{ mb: 3 }}>
                ...
                <Typography variant="body2">This item has been marked as damaged and will be placed in the DAMAGE BIN.</Typography>
                {damage_notes && <Typography variant="body2" sx={{ mt: 1 }}>Notes: {damage_notes}</Typography>}
              </Alert>
            )}
            ...
            <Box sx={{ mt: 3, p: 3, bgcolor: 'grey.50', borderRadius: 2, border: '1px solid', borderColor: 'grey.300' }}>
              ...
              <Typography variant="body1">Copy ID: {transaction_info.copy_id}</Typography>
              ...
              <Typography variant="body1">Patron: {transaction_info.patron_name}</Typography>
              <Typography variant="body1" sx={{ mb: 2 }}>ID: {transaction_info.patron_id}</Typography>
              ...
              <Typography variant="body1">Due Date: {format_date(transaction_info.due_date)}</Typography>
              {transaction_info.is_overdue && !is_damaged ? (
                <Box sx={{ p: 2, mt: 2, bgcolor: 'error.50', borderRadius: 1, border: '2px solid', borderColor: 'error.main' }}>
                  ...
                  <Typography variant="body1" fontWeight="bold" color="error.main" sx={{ mt: 1 }}>
                ...
                <Box sx={{ p: 2, mt: 2, bgcolor: 'success.50', borderRadius: 1, border: '1px solid', borderColor: 'success.main' }}>
                  <Typography variant="body1" fontWeight="bold" color="success.main">
                    Item returned on time - No late fees
                  </Typography>
                </Box>
              ) : (
                <Box sx={{ p: 2, mt: 2, bgcolor: 'warning.50', borderRadius: 1, border: '1px solid', borderColor: 'warning.main' }}>
            ...
            <Button
              ...
              startIcon={processing_return ? <CircularProgress size={20} /> : null}
              size="large"
              sx={{ mt: 3 }}
            >
              {processing_return ? 'Processing...' : is_damaged ? 'Mark as Damaged & Complete Return' : 'Complete Return'}
            </Button>
          </Box>
        )}
        ...
        {active_step === 3 && transaction_info && (
          <Box>
            ...
            <Box sx={{ mt: 3, p: 3, bgcolor: is_damaged ? 'error.50' : 'success.50', borderRadius: 2, border: '2px solid', borderColor: is_damaged ? 'error.main' : 'success.main' }}>
              <Typography variant="h6" gutterBottom fontWeight="bold" color="text.primary">
                RETURN CONFIRMATION
              </Typography>
              
              <Typography variant="body1" color="text.primary">Item ID: {transaction_info.copy_id}</Typography>
              <Typography variant="body1" color="text.primary">Title: {transaction_info.item_title}</Typography>
              <Typography variant="body1" color="text.primary" sx={{ mb: 2 }}>Patron: {transaction_info.patron_name}</Typography>
              ...
              <Box sx={{ p: 2, bgcolor: needs_transfer ? 'error.50' : 'warning.50', borderRadius: 1, border: '2px solid', borderColor: needs_transfer ? 'error.main' : 'warning.main' }}>
                <Typography variant="h6" fontWeight="bold" color={needs_transfer ? 'error.main' : 'warning.main'}>
                  📦 BIN LOCATION
                </Typography>
                <Typography variant="body1" fontWeight="bold" fontSize="1.5rem" color="text.primary" sx={{ my: 1 }}>
