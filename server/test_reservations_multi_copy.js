...
async function test_three_reservations_multi_copy() {
  try {
    console.log('🧪 Testing 3 reservations for an item with 2 copies...\n');

    // Wait for database to initialize
    const database = await db();

    // Find a library item with 2 available copies
    ...
    if (available_items.length === 0) {
      console.log('❌ No items found with exactly 2 available copies');
      process.exit(1);
    }
    ...
    for (let i = 0; i < patron_ids.length; i++) {
      ...
      console.log(`   Available copies before: ${available_copies.length}`);
      ...
      if (reservation_status === 'ready' && available_copies.length > 0) {
        ...
        console.log(`   ✓ Copy ${available_copies[0].id} marked as Reserved\n`);
      }
    ...
    all_reservations.forEach((res, idx) => {
      ...
      console.log(`${emoji} ${idx + 1}. ${res.patron_name} (Queue #${res.queue_position})`);
    ...
    const copies = await execute_query(
      'SELECT id, status FROM LIBRARY_ITEM_COPIES WHERE library_item_id = ?',
      [test_item.id]
    );

    copies.forEach((copy) => {
      console.log(`Copy ${copy.id}: ${copy.status}`);
    });

    // Check for the bug - ANY reservation with queue position 1 should have "ready" status
    const position_one_reservations = all_reservations.filter(r => r.queue_position === 1);

    console.log('\n═══════════════════════════════════════════════════');
    console.log('BUG CHECK');
    console.log('═══════════════════════════════════════════════════\n');

    position_one_reservations.forEach((res) => {
      if (res.status === 'waiting') {
        console.log(`❌ BUG DETECTED: ${res.patron_name} at position #${res.queue_position} has "waiting" status instead of "ready"!`);
      } else if (res.status === 'ready') {
        console.log(`✅ CORRECT: ${res.patron_name} at position #${res.queue_position} has "ready" status`);
      }
    });
...
test_three_reservations_multi_copy();
