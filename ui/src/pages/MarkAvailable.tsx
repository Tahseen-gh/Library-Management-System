...
export const MarkAvailable: React.FC = () => {
  ...
  const fetch_returned_items = async () => {
    ...
    try {
      ...
      const returned_response = await fetch(`${API_BASE_URL}/item-copies?status=returned`);
      const damaged_response = await fetch(`${API_BASE_URL}/item-copies?status=damaged`);
      ...
      // Filter to only show items that belong to this branch (can be reshelved here)
      const reshelve_able = all_items.filter((item: any) => item.branch_id === selectedBranch.id);

      console.log('Reshelve-able items:', reshelve_able);

      set_returned_items(reshelve_able);
    } catch (err) {
      set_error(err instanceof Error ? err.message : 'Failed to load returned items');
    } finally {
      set_loading(false);
    }
  };
  ...
  const mark_available = async (copy_id: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error_data.message || error_data.error || 'Failed to mark item as available');
      }
      ...
      set_error(err instanceof Error ? err.message : 'Failed to mark item as available');
    } finally {
  ...
  const mark_available_by_id = async () => {
    if (!item_id_input.trim() || !selectedBranch) return;
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Scan or select items from the reshelve bin to mark them as available for checkout.
        </Typography>
        <Chip 
          label={`Current Branch: ${selectedBranch.branch_name}`} 
          color="primary" 
          sx={{ mt: 2 }}
        />
      </Box>
                  ...
                  <ListItemText
                    primary={
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        {item.status === 'damaged' && (
                          <Chip label="DAMAGED - REPAIRED" color="error" size="small" />
                        )}
                      </Box>
                    }
                    secondary={
                      <Box sx={{ mt: 1 }}>
                        ...
                        {item.status === 'damaged' && (
                          <Typography variant="body2" color="error.main" sx={{ mt: 0.5 }}>
                            Damaged - Mark as available only after repair/replacement
                          </Typography>
                        )}
                        {item.return_branch_id && item.return_branch_id !== item.branch_id && (
                          <Chip
                            icon={<Warning />}
