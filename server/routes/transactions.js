...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Create transaction (id will be auto-generated by AUTOINCREMENT)
      ...
      await db.update_record('LIBRARY_ITEM_COPIES', parseInt(copy_id), {
        status: 'borrowed',
        updatedAt: checkout_date,
      });
...
router.post(
  ...
  [
    ...
    body('return_branch_id').notEmpty().withMessage('Return branch ID is required'),
    body('new_condition')
      .optional()
  ...
  async (req, res) => {
    try {
      const { copy_id, return_branch_id, new_condition, notes } = req.body;
      ...
      // Get the item copy to check its home branch
      const item_copy = await db.get_by_id('LIBRARY_ITEM_COPIES', parseInt(copy_id));
      
      // Determine if item needs inter-branch transfer
      const needs_transfer = item_copy.branch_id !== parseInt(return_branch_id);
      
      // Update library item copy status
      // If already marked as damaged, keep that status; otherwise set to 'returned'
      const update_data = {
        status: item_copy.status === 'damaged' ? 'damaged' : 'returned',
        return_branch_id: needs_transfer ? parseInt(return_branch_id) : null,
        condition: new_condition || transaction.condition,
        updatedAt: return_date,
      };

      await db.update_record('LIBRARY_ITEM_COPIES', parseInt(copy_id), update_data);

      // Create fine record if there's a fine - US 3.1
      if (fine_amount > 0) {
        const fine_data = {
          transaction_id: parseInt(transaction.id),
          patron_id: parseInt(transaction.patron_id),
          amount: fine_amount,
      ...
      const return_branch = await db.get_by_id('BRANCHES', parseInt(return_branch_id));
      const home_branch = await db.get_by_id('BRANCHES', item_copy.branch_id);
        ...
        data: {
          ...
          days_overdue:
            fine_amount > 0
              ...
              : 0,
          needs_transfer,
          return_branch: return_branch?.branch_name,
          home_branch: home_branch?.branch_name,
          bin_label: needs_transfer ? `TRANSFER TO: ${home_branch?.branch_name}` : 'RESHELVE BIN',
        },
      });
    } catch (error) {
      res.status(500).json({
...
// POST /api/v1/transactions/reshelve - Reshelve item (mark as available after physical shelving)
// Changes item status from 'returned' to 'available'
// Only works if item is at its home branch
router.post(
  '/reshelve',
  [
    body('copy_id').notEmpty().withMessage('Copy ID is required'),
  ...
  async (req, res) => {
    try {
      ...
      const library_item_copy = await db.get_by_id('LIBRARY_ITEM_COPIES', parseInt(copy_id));
      ...
      // Verify item is in 'returned' or 'damaged' status
      if (library_item_copy.status !== 'returned' && library_item_copy.status !== 'damaged') {
        return res.status(400).json({
          error: `Item cannot be reshelved. Current status: ${library_item_copy.status}. Only items with 'returned' or 'damaged' status can be reshelved.`,
        });
      }

      // Verify item is being reshelved at its home branch
      if (library_item_copy.branch_id !== parseInt(branch_id)) {
        const home_branch = await db.get_by_id('BRANCHES', library_item_copy.branch_id);
        const current_branch = await db.get_by_id('BRANCHES', parseInt(branch_id));
        
        return res.status(400).json({
          error: 'Item cannot be reshelved at this branch',
          message: `This item belongs to ${home_branch?.branch_name}. It must be transferred there before reshelving.`,
          home_branch: home_branch?.branch_name,
          current_branch: current_branch?.branch_name,
        });
      }

      // Update library item copy status to 'available' and clear return_branch_id
      ...
      res.json({
        success: true,
        message: 'Item reshelved successfully and is now available',
        data: {
          copy_id,
          status: 'available',
